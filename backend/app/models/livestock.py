...
class Animal(Base):
    ...
    tag_id: Mapped[str] = mapped_column(String(64), unique=True, index=True)  # INAPH / RFID Tag
...
class VeterinaryDrug(Base):
    ...
    active_ingredient: Mapped[str] = mapped_column(String(128), index=True)  # e.g., Enrofloxacin
    who_classification: Mapped[str] = mapped_column(String(64))             # CIA, HPCI, or Standard
    mrl_limit_ppm: Mapped[float] = mapped_column(Float, nullable=False)     # FSSAI prescribed MRL
...
class TreatmentRecord(Base):
    ...
    rvp_license_number: Mapped[str] = mapped_column(String(64), nullable=False) # Vet license
